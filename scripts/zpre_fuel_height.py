...
def move_rod(pos,distance,start):
    ...
    # move rod
    # 1, 65, 91 are the volume ids of the control rod assembly (identified via cubit gui)
    ...
    cad_to_h5m(h5m_filename = h5m_out_filepath + '_pos_'+str(pos-1./2.54)[0:2]+'.h5m',
            ...
            files_with_tags=[{"cad_filename": "./step_files/fuel_height/zpre_fuel_"+str(start)+".step",
                              "transforms":{'scale':scale,'move':([1,65,91],[0,0,-pos*2.54-distance])}},
                            ],
...
sp = openmc.StatePoint(zero_model.run(output=True))
